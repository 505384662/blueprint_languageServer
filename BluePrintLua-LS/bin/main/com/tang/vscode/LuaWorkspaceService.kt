...
class LuaWorkspaceService : WorkspaceService, IWorkspace {
    ...
    fun initConfigFiles(files: Array<BluePrintConfigurationSource>) {
        configurationManager.init(files)
    }

    @JsonRequest("bluePrint/updateConfig")
    ...
    private fun loadWorkspace(monitor: IProgressMonitor) {
        ...
        for (collection in collections) {
            ...
            for (uri in collection.files) {
                ...
                if (file != null) {
                    monitor.setProgress(
                        "BluePrint parse file[${(processedCount / totalFileCount * 100).toInt()}%]: ${file.canonicalPath}",
