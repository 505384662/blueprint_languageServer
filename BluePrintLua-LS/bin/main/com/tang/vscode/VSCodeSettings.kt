...
object VSCodeSettings : IVSCodeSettings {
    ...
    fun update(settings: JsonObject): SettingsUpdateResult {
        ...
        //case sensitive
        val caseSensitive = path("bluePrintlua.completion.caseSensitive")
        ...
        // show codeLens
        myShowCodeLens = path("bluePrintlua.codeLens")?.asBoolean == true

        path("bluePrintlua.hint.paramHint")?.asBoolean?.let {
            LuaSettings.instance.paramHint = it
        }

        path("bluePrintlua.hint.localHint")?.asBoolean?.let {
            LuaSettings.instance.localHint = it
        }

        path("bluePrintlua.hint.varargHint")?.asBoolean?.let {
            LuaSettings.instance.varargHint = it
        }

        path("bluePrintlua.hint.overrideHint")?.asBoolean?.let {
            LuaSettings.instance.overrideHint = it
        }

        path("bluePrintlua.constructorNames")?.asString?.let {
            LuaSettings.instance.constructorNames = it.split(";").filter { it.isNotEmpty() }.toTypedArray()
        }

        path("bluePrintlua.requireLikeFunctions")?.asString?.let {
            LuaSettings.instance.requireLikeFunctions = it.split(";").filter { it.isNotEmpty() }.toTypedArray()
        }


        path("bluePrintlua.typeSafety.anyTypeCanAssignToAnyDefineType")?.asBoolean?.let {
            DiagnosticsOptions.anyTypeCanAssignToAnyDefineType = it
        }
        path("bluePrintlua.typeSafety.defineAnyTypeCanBeAssignedByAnyVariable")?.asBoolean?.let {
            DiagnosticsOptions.defineAnyTypeCanBeAssignedByAnyVariable = it
        }
        path("bluePrintlua.typeSafety.defineTypeCanReceiveNilType")?.asBoolean?.let {
            DiagnosticsOptions.defineTypeCanReceiveNilType = it
        }

        path("bluePrintlua.inspections.parameterValidation")?.asString?.let {
            DiagnosticsOptions.parameterValidation = InspectionsLevel.valueOf(it)
        }
        path("bluePrintlua.inspections.fieldValidation")?.asString?.let {
            DiagnosticsOptions.fieldValidation = InspectionsLevel.valueOf(it)
        }
        path("bluePrintlua.inspections.undeclaredVariable")?.asString?.let {
            DiagnosticsOptions.undeclaredVariable = InspectionsLevel.valueOf(it)
        }
        path("bluePrintlua.inspections.assignValidation")?.asString?.let {
            DiagnosticsOptions.assignValidation = InspectionsLevel.valueOf(it)
        }
        path("bluePrintlua.inspections.deprecated")?.asBoolean?.let {
    ...
    private fun updateSourceRoots() {
        mySourceRoots.clear()
        val sourceRoots = path("bluePrintlua.source.roots")
